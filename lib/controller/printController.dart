...
import 'package:product_ms/controller/initPrinter.dart';
...
printingTokenForTAForUser(printer, data, companyInfo, isUpdate) {
  ...
  printer.text('Total Amount   : Rs.${data[0]['gross_amount']}');
  if (data[0]['discount'] != '' && data[0]['discount'] != "0.0") {
    printer.text('Disount Amount : Rs.${data[0]['discount'].toString()}');
  }
  printer.text('Net Amount     : Rs.${data[0]['net_amount'].toString()}');
  printer.text('Date Time      : $currentTime');
  printer.text('------------------------------------------------');
  printer.row([
    PosColumn(
...
// printingOrderReceiptforTABLE(printer, data, storeName, List orderItem, bool isUpate) {
//   dynamic currentTime =
//       DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now());
//   printer.text('');
//   if (isUpate) {
...
Future<bool> printerHomeDelivery(data, qrImage, context) async {
  ...
  if (res == PosPrintResult.success) {
    printerHomeDeliveryBill(data, companyInfo, qrImage, printer);
    printer.disconnect();
    return true;
  } else {
    printer.disconnect();
...
// Future<image.Image?> resizeImg(qrImage) async {

//   final ByteData data = await NetworkAssetBundle(Uri.parse(qrImage)).load("");
...
printerHomeDeliveryBill(data, companyInfo, qrImage, printer) async {
  dynamic currentTime =
      DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now());
  printer.text('');
  ...
  printer.text('');
  printer.text('Ref.Code      : ${data['hd_no']}',
      styles: const PosStyles(bold: true));
  printer.text('bill_no       : ${data['bill_no']}');
  printer.text('Customer Name : ${data['customer_name']}');
  printer.text('Mobile        : ${data['phone_no']}');
  printer.text('Address       : ${data['address']}');
  printer.text('Date Time     : $currentTime');
  printer.text('------------------------------------------------');
  printer.row([
    PosColumn(
  ...
  printer.text('Visit again soon...',
      styles: const PosStyles(align: PosAlign.center));
  printer.text('------------------------------------------------');
  printer.text('Download our App ',
      styles: const PosStyles(align: PosAlign.center));
  printer.text('Find us on Play Store && App Store',
      styles: const PosStyles(align: PosAlign.center));
  printer.text('------------------------------------------------');
  printer.beep(n: 1);
  printer.feed(2);
  ...
  final ByteData imageData = await rootBundle.load('assets/rabbit_black.jpeg');
  final Uint8List bytes = imageData.buffer.asUint32List() as Uint8List;
  // final Image? image = decodeImage(bytes);
  // printer.image(image);
